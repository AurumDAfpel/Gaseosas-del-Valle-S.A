...
READS SQL DATA
BEGIN
    DECLARE  id_pedido DECIMAL(12,2);
...
SELECT
    ...
    COUNT(p.id_pedido) AS total_pedidos
FROM clientes c
INNER JOIN pedidos p ON c.id_sede   = p.id_sede  
GROUP BY c.id_sede  , c.nombre_sede , c.ubicacion ,
         c.telefono, c.correo_electronico;

        no recuerdo nada toy estresado perdon...